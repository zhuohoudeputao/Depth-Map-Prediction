...
def conv_theano_mm(x, k, border_mode, transpose=False, stride=1):
    ...
    if transpose:
        ...
        res = theano.sandbox.cuda.blas.GpuCorrMM_gradInputs(
            ...
            subsample=(stride, stride))(k, x, shape=bottom_shape)
...
class MachinePart(object):
    ...
    def __init__(self, vars, exclude=('self',)):
        ...
        self.vars = dict((k, v) for (k, v) in iter(vars.items())
...
class Unit(object):
    ...
    def init_params(self, *args, **kwargs):
        ...
        * initialize by calling unit init code (since no was file specified)
        ...
        for (k, x) in self.tie_params.items():
    ...
    def _save_params(self, dir=None, fn=None, attrs=[]):
        ...
        with logutil.open(fn, 'w') as f:
            ...
            pickle.dump(pdict, f, pickle.HIGHEST_PROTOCOL)
    ...
    def get_updates(self, cost, learning_rate, momentum):
        ...
        for (p, constraint) in self.constraints.items():

            pupdates[p] = constraint(pupdates[p])

        return OrderedDict(list(gupdates.items()) + list(pupdates.items()))
...
class MaxPool(Unit):
    ...
    def pool(self, y):
        ...
        (p_y, p_inds) = pooling.maxpool2d(
            y, winsize=self.poolsize, stride=self.poolstride)
